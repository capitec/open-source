...
test(`Chip - Closable Behaviour`, async ({ page, isMobile }) => {
    await withCoverage(page, async () => {
        await page.goto('/components/chip/');
        const args = await page.locator('story-renderer[key=Closable]').evaluate((storyRenderer) => storyRenderer.story.args);
        ...
        await chip.evaluate((node) => {
            node.addEventListener('remove', () => window.jestRemove());
        });
...
test(`Chip - Icon Slot`, async ({ page, isMobile }) => {
    await withCoverage(page, async () => {
        await page.goto('/components/chip/');
        const args = await page.locator('story-renderer[key=Chip_Slot_Icon]').evaluate((storyRenderer) => storyRenderer.story.args);
...
test(`Chip - Close Icon Slot`, async ({ page, isMobile }) => {
    await withCoverage(page, async () => {
        await page.goto('/components/chip/');
        const args = await page.locator('story-renderer[key=Custom_Close_Icon]').evaluate((storyRenderer) => storyRenderer.story.args);
