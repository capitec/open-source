...
const stringItems = ['Bruce Wayne', 'Clark Kent', 'Barry Allen', 'Arthur Curry', 'Hal Jordan'];    
const App = () => <OmniSelect label="${e.label}" display-field="${e.displayField}" id-field="${e.idField}" items={stringItems} />;`}],name:"String",description:"Use a string array as the items source.",args:{label:"String",data:{},items:S,displayField:"label",idField:"id"},play:async e=>{var n;let t=h(e.canvasElement).getByTestId("test-select"),l=y.fn(),i=y.fn();t.addEventListener("click",l),t.addEventListener("change",i),await r.click(t);let s=await o(t.shadowRoot,".item");await r.click(s);let m=(n=t.shadowRoot)==null?void 0:n.getElementById("select");await a(m).toHaveValue(S[0])}},we={render:e=>p`
        <omni-select
            data-testid="test-select"
            label="${c(e.label)}"
            .items="${e.items}"
...
`,frameworkSources:[{framework:"HTML",load:e=>`<omni-select id='omni-select' label="${e.label}" searchable></omni-select>
    ...
    <\/script>`},{framework:"React",load:e=>`import { OmniSelect } from "@capitec/omni-components-react/select";
        ...
        </OmniSelect>`}],name:"Custom Search",description:"Custom search function",args:{label:"Custom Search",searchable:!0,items:S,filterItems:P},play:async e=>{var w,u;let t=h(e.canvasElement).getByTestId("test-select"),l=y.fn();t.addEventListener("click",l),await r.click(t);let i=await o(t.shadowRoot,"#searchField");await a(i).toBeTruthy(),B(i),await r.click(i);let s="Bruce";await r.type(i,s);let m=await o(t.shadowRoot,"#items-container");await a(m).toBeTruthy();let n=(w=t.shadowRoot)==null?void 0:w.getElementById("items");await a(n).toBeTruthy();let d=await o(t.shadowRoot,".item");await a(d).toBeTruthy(),await r.click(d);let f=(u=t.shadowRoot)==null?void 0:u.getElementById("select");await a(f).toHaveValue(S[0])}},Se={render:e=>p`
    <omni-select data-testid="test-select" label="${c(e.label)}" .items="${e.items}" ?searchable="${e.searchable}" .filterItems="${e.filterItems}">
    </omni-select>
`,frameworkSources:[{framework:"HTML",load:e=>`<omni-select id='omni-select' label="${e.label}" searchable></omni-select>
    <script defer>
    ...
    <\/script>`},{framework:"React",load:e=>`import { OmniSelect } from "@capitec/omni-components-react/select";
        ...
        const App = () => <OmniSelect label="${e.label}" items={stringItems} filterItems={customSearch} searchable></OmniSelect>`}],name:"Server Side Filtering",description:"Select with server side filtering",args:{label:"Server Side Filtering",searchable:!0,items:S,filterItems:_},play:async e=>{var w,u;let t=h(e.canvasElement).getByTestId("test-select"),l=y.fn();t.addEventListener("click",l),await r.click(t);let i=await o(t.shadowRoot,"#searchField");await a(i).toBeTruthy(),B(i),await r.click(i);let s="Bruce";await r.type(i,s);let m=await o(t.shadowRoot,"#items-container");await a(m).toBeTruthy();let n=(w=t.shadowRoot)==null?void 0:w.getElementById("items");await a(n).toBeTruthy();let d;navigator.userAgent==="Test Runner"?d=await o(t.shadowRoot,".item",void 0,3e3):d=await o(t.shadowRoot,".item",void 0,5e3),await a(d).toBeTruthy(),await r.click(d);let f=(u=t.shadowRoot)==null?void 0:u.getElementById("select");await a(f).toHaveValue(S[0])}},ge={render:e=>p`
...
`,frameworkSources:[{framework:"HTML",load:e=>`
    ...
    <\/script>`},{framework:"React",load:e=>`import { OmniSelect } from "@capitec/omni-components-react/select";
        ...
        <svg slot="search-clear" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24px" height="24px" style="fill: orange;"><path d="M12 2.25c5.385 0 9.75 4.365 9.75 9.75s-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12 6.615 2.25 12 2.25Zm0 1.5a8.25 8.25 0 1 0 0 16.5 8.25 8.25 0 0 0 0-16.5ZM12 7a.75.75 0 0 1 .75.75v3.5h3.5a.75.75 0 0 1 .743.648L17 12a.75.75 0 0 1-.75.75h-3.5v3.5a.75.75 0 0 1-.648.743L12 17a.75.75 0 0 1-.75-.75v-3.5h-3.5a.75.75 0 0 1-.743-.648L7 12a.75.75 0 0 1 .75-.75h3.5v-3.5a.75.75 0 0 1 .648-.743Z"/></svg>
        </OmniSelect>`}],name:"Custom Search Clear Icon",description:"Select component with a custom search field clear icon",args:{label:"Custom Search Icon",searchable:!0,items:S},play:async e=>{var w,u;let t=h(e.canvasElement).getByTestId("test-select"),l=y.fn();t.addEventListener("click",l),await r.click(t);let i=await o(t.shadowRoot,"#searchField");await a(i).toBeTruthy(),B(i),await r.click(i);let s="Bruce";await r.type(i,s);let m=await o(t.shadowRoot,"#items-container");await a(m).toBeTruthy();let n=(w=t.shadowRoot)==null?void 0:w.getElementById("items");await a(n).toBeTruthy();let d=await o(t.shadowRoot,".item");await a(d).toBeTruthy(),await r.click(d);let f=(u=t.shadowRoot)==null?void 0:u.getElementById("select");await a(f).toHaveValue(S[0])}},ve=F("omni-select"),Ie=A("omni-select"),Be=x("omni-select"),$e=M("omni-select"),ke=R("omni-select"),Te=L("omni-select"),Ce=E("omni-select");export{de as Async_Per_Item,$e as Clearable,ke as Custom_Clear_Slot_Icon,he as Custom_Control_Slot,be as Custom_Search,ge as Custom_Search_Icon,ue as Disabled,we as Empty_Message,Be as Error_Label,Ie as Hint,me as Interactive,ve as Label,pe as Loading_Slot,Te as Prefix,fe as Searchable,Se as Server_Side_Filtering,ye as String_Array,Ce as Suffix,ce as default};
