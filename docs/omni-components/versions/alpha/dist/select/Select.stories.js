...
`,frameworkSources:[{framework:"HTML",load:e=>`<omni-select id='omni-select' label="${e.label}" display-field="${e.displayField}" id-field="${e.idField}" searchable></omni-select>
    ...
    `,frameworkSources:[{framework:"HTML",load:e=>`<omni-select id='omni-select' label="${e.label}" display-field="${e.displayField}" id-field="${e.idField}" searchable></omni-select>
        ...
        const App = () => <OmniSelect label="${e.label}" display-field="${e.displayField}" id-field="${e.idField}" items={stringItems} searchable></OmniSelect>`}],name:"Custom Search Clear ICon",description:"Select component with a custom search field clear icon",args:{label:"Custom Search Clear Icon",displayField:"label",idField:"id",searchable:!0,items:S,filterItems:B},play:async e=>{var u,w;let t=f(e.canvasElement).getByTestId("test-select"),a=y.fn();t.addEventListener("click",a),await r.click(t);let l=await o(t.shadowRoot,"#searchField");await i(l).toBeTruthy(),$(l),await r.click(l);let s="Bruce";await r.type(l,s);let d=await o(t.shadowRoot,"#items-container");await i(d).toBeTruthy();let n=(u=t.shadowRoot)==null?void 0:u.getElementById("items");await i(n).toBeTruthy();let m=await o(t.shadowRoot,".item");await i(m).toBeTruthy(),await r.click(m);let h=(w=t.shadowRoot)==null?void 0:w.getElementById("select");await i(h).toHaveValue(S[0])}},ve=A("omni-select"),Ie=L("omni-select"),$e=x("omni-select"),Be=M("omni-select"),ke=H("omni-select"),Fe=E("omni-select"),Te=R("omni-select");export{me as Async_Per_Item,Be as Clearable,ke as Custom_Clear_Slot_Icon,fe as Custom_Control_Slot,ge as Custom_Search_Clear_Icon,be as Custom_Search_Function,we as Disabled,ue as Empty_Message,$e as Error_Label,Ie as Hint,de as Interactive,ve as Label,pe as Loading_Slot,Fe as Prefix,he as Searchable,Se as Server_Side_Filtering,ye as String_Array,Te as Suffix,ce as default};
